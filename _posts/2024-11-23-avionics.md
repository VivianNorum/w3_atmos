...
For the onboard computer, we recommend to install it in the location suggested in the 3D model, that is, in front of the Pixhawk Mini. In our case, we use a Jetson Orin NX and we provide a simple box to hold it in place in the STLs below. Note that you need a power regulator (item 0004) to power the onboard computer from battery power. We recommend to connect the regulator to the XT60 hub, and adjust its output to 15V for the Jetson Orin NX (or the specified voltage for your onboard computer).
...
The last step is to install the aluminium profiles for payload support. Each vertical aluminium profile should have a height of 13cm or higher (depending on the user application). To that end, we recommend to install the profiles in the locations shown in the 3D model below. We recommend to use the provided angle brackets and T-slot nuts to secure the profiles to the plate.
